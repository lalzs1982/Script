...
colnames(res)=c("Study","Phenotype","Region","RegDNMs","TotalDNMs","RegDNMrate","TotalDNMrate", "Expected_RegDNMs","Enrichment(Obs/Exp)","p(binom.test)")
    ...
    for (ph in unique(data[data[,2]==study,3]))
    {
        ...
        if(nrow(data1_total)==1 && nrow(data1_reg)>0)
        {
            ...
            for(i in 1:nrow(data1_reg))
            {
                ...
                res=rbind(res,c(study,ph,as.character(data1_reg[i,1]),reg_obs,total_obs,reg_mr,total_mr,reg_exp,reg_obs/reg_exp,test$p.value))
            }
        }
    }
}

res=cbind(res,p.adjust(as.numeric(res[,10]),method='BH'))
colnames(res)[11]="FDR"
res=res[order(log10(as.numeric(res[,10]))),]
write.table(res,paste(files[1],".dnm.enrich",sep=''),sep="\t",row.names=F,quote=F)

#make plot
res1=res[order(as.numeric(res[,10]),decreasing=T),]
#res1=res[1:6,]
res1=res1[as.numeric(res1[,4])>=10,]
...
for(st in unique(res1[,1]))
{
    for(ph in unique(res1[,2]))
    {
        ...
        if(nrow(res2)>1)
        {
        plot(x=1:nrow(res2),y=as.numeric(res2[,9]), col='red',main=paste(st, ph),ylab="Fold Change",xlab="Samples",ylim=c(0,max(10,as.numeric(res2[,9]))),xlim=c(-2,nrow(res2)+2))
        text(x=1:nrow(res2),y=as.numeric(res2[,9]),labels=paste(as.character(res2[,3]),round(as.numeric(res2[,10]),4),sep="\n"),cex=0.5,srt=30)
        abline(h=1,lty=2)
        }
    }
}
