...
coor=$1 # gtf file
awk 'BEGIN{FS="\t";OFS="\t"}$3=="exon"{print $1,$4-1,$5,$1":"$4":"$5":"$7,0,$7}' $coor |sort|uniq > $coor.0
...
perl -e 'while(<>){chomp;my @x=split/\t/;next if $x[2] ne "exon";$_=~/transcript_id\s\"(.*?)\"/;my $ti=$1;print join("\t",@x[0,3,4],$ti,0,$x[6]),"\n"}' $coor > $coor.1
