...
out="/project2/xinhe/zhongshan/data/transcript_anno/hg19_refGenes"
...
#produce all mutations from gtf and genome sequences
~/code/mutation_annotation/longest_transc.pl $transc_coors > $transc_coors.lg #for efficiency, we only use longest transcripts
#sbatch ~/code/mutation_annotation/Allmutation_introduce.sbatch $transc_coors.lg $hg19 #obtain $transc_coors.Allmut and $transc_coors.transc.fa
#mv $transc_coors.Allmut $outdir/
allmut='/project2/xinhe/zhongshan/data/transcript_anno/hg19_refGenes_exons.gtf.Allmut'

#obtain unique genomic coodrinates
#cut -f 6 $allmut | uniq|sort|uniq > ./tt
#for chr in `cat ./tt`
#do
#    echo "grep $chr $allmut | cut -f 6,7,8,9,10,11 |uniq | sort|uniq >> $allmut.gcoor"
#    grep $chr $allmut | cut -f 6,7,8,9,10,11 |uniq | sort|uniq >> $allmut.gcoor
#done

#allmut_ontranscript='' #alonng with corresponding genomic coordinates
#/home/zhongshan/code/transcriptpos2genomepos.sh $transc_coors # obtain 
